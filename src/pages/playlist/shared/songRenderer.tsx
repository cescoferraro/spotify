...
export const SongRenderer = (
  {refresh, liked, player, list, loading}:
    { loading: boolean, refresh: () => void, liked: string[], player: Player, list: Song[] }
) =>
  ({key, index, style}: any) => {
    ...
    return (
      <ListItem key={key} style={style}>
        ...
        <Box style={{minWidth: 72}}>
          <PlaylistPlayButton uri={uri} url={url} player={player}/>
          <LoveSongsButton loading={loading} refresh={refresh} ids={liked} listElement={listElement}/>
        </Box>
      </ListItem>
    );
  };
