...
function SpotifyBackButton(props: { onClick: (() => void) | undefined }) {
  return <IconButton
...
const QueryInput = (props: {
  ...
  return <div style={{width: "100%"}}>
    {props.searchable && <Paper style={{background: "#8E8D8D", display: "flex", alignItems: "center"}}>
      ...
      <Box boxShadow={searchable ? 3 : 0}>
        <AppBar classes={{root: styles.root}}>
          <Toolbar style={{paddingLeft: 8, paddingRight: 8}}>
            <SpotifyBackButton onClick={onClick}/>
            <QueryInput searchable={searchable} value={query} setQuery={setQuery} placeholder={label}/>
            <LoginAvatar auth={auth}/>
          </Toolbar>
        </AppBar>
      </Box>
    </HideOnScroll>
